...
export function renderMarkdown(report) {
  ...
  const failed = report.repositories.filter(repo => repo.latestRun?.conclusion === "failure").length;
  const successful = report.repositories.filter(repo => repo.latestRun?.conclusion === "success").length;
  return [
...
async function githubJson(path, token, params = {}) {
  ...
  if (!response.ok) throw new Error(`${response.status}: ${payload.message || "GitHub API request failed"}`);
  return payload;
}
...
async function inspectRepository(repo, token) {
  try {
    const [runs, pulls] = await Promise.all([
      githubJson(`/repos/${repo.full_name}/actions/runs`, token, { branch: repo.default_branch, per_page: 1 }),
      githubJson(`/repos/${repo.full_name}/pulls`, token, { state: "open", per_page: 100 }),
    ]);
    return summarizeRepository(repo, runs.workflow_runs?.[0] || null, pulls, null);
  } catch (error) {
    return summarizeRepository(repo, null, [], error instanceof Error ? error.message : String(error));
  }
}
...
export async function collectReport(env = process.env) {
  ...
  for (let index = 0; index < repositories.length; index += 8) {
    ...
    results.push(...await Promise.all(batch.map(repo => inspectRepository(repo, token))));
  }
...
async function main() {
  ...
  console.log(`Inventoried ${report.repositories.length} repositories; reports written to ${outputDir}`);
}
