...
const recordFile =
  process.env.RECORD_FILE || join(outputDir, "execution-record.jsonl");
...
export function safeSummary(report) {
  ...
  return {
    ...
    openPullRequests,
    scopeLimited: Boolean(report.scope?.warning),
    scopeWarning: report.scope?.warning || null,
  };
}

async function main() {
  const state = await readState();
  const previousNumber = Number.isInteger(state.executionNumber)
  ...
  if (previousNumber >= MAX_EXECUTIONS || state.status === "complete") {
    const completedState = {
      ...
      nextRecommendedAction:
        "No further hourly repair cycle is required; perform health checks only if this workflow is intentionally re-enabled.",
    };
  ...
  const healthy =
    summary.latestDefaultBranchFailures === 0 &&
    summary.repositoryInspectionErrors === 0 &&
    summary.scopeLimited === false;
  ...
  const nextState = {
    ...
    remainingBlockers: [
      ...(summary.repositoryInspectionErrors > 0
        ? [
            "One or more repositories could not be inspected with the available read-only authorization.",
          ]
        : []),
      ...(summary.scopeWarning ? [summary.scopeWarning] : []),
    ],
    nextRecommendedAction,
  };
  const record = {
  ...
  await writeFile(
    join(outputDir, "continuation-state.json"),
